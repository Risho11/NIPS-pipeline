...
def current_ranges(locked_additive_wt=None) -> str:
    ...
    stock_rule = (
        f' When "polymer_wt" equals the polymer stock concentration '
        f'({bounds.polymer_stock_concentration()} wt%) and "additive_wt" is 0, '
        'the solution is undiluted polymer stock: "mixing_temp" must be 25 degrees Celsius '
        'because no mixing occurs. This constraint overrides historical temperatures, '
        'including when repeating a historical candidate for validation. '
        'A blend containing additive does not automatically skip mixing at maximum polymer concentration.'
    )
    return _BASE_RANGES + tri_ranges + stock_rule
#-----------#
