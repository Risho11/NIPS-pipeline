# protocol-dryrun.py
#
# Dry-run variant of protocol-multithreaded.py for testing the opentrons
# (and optionally other) hardware quickly, without running through the
# full compression-tester cycle (3 zero tests + 3 membrane tests), which
# is normally the slowest part of a run.
#
# See TESTS/DRY_RUN_README.md for the full explanation of the DRY_RUN_*
# flags below, how this file relates to protocol-multithreaded.py, and how
# to run/trigger it.
#
# Usage (on the robot/jupyter machine, same as protocol-multithreaded.py):
#   python protocol-dryrun.py
# Then from the minipc, trigger a run with:
...
# Opentrons (mixing, dispensing, pullcast) ALWAYS runs for real - that's the
# point of this script. The flags below control whether the *other* hardware
# subsystems run for real or are skipped/stubbed out.

# Skip the compression-tester arm choreography (both the 3 "zero" tests done
# on a fresh coupon before placing it on the opentrons stand, and the 3
# "membrane" tests done after the NIPS bath), and skip the arduino/url calls
# used to run/poll the compression tester.
DRY_RUN_COMPRESSION_TESTS = True

# Skip chiller.go_to_temperature() - proceed as if the chiller were already
# at temperature.
DRY_RUN_CHILLER = False

# Skip the delayed knife-cleaning background task.
DRY_RUN_KNIFE_CLEANING = False

# Skip url.take_snapshot() calls (pre- and post-bath photos).
DRY_RUN_CAMERA = False
# ─────────────────────────────────────────────────────────────────────────
...
print("\n*** DRY RUN MODE ***")
print("DRY_RUN_COMPRESSION_TESTS = " + str(DRY_RUN_COMPRESSION_TESTS))
print("DRY_RUN_CHILLER = " + str(DRY_RUN_CHILLER))
print("DRY_RUN_KNIFE_CLEANING = " + str(DRY_RUN_KNIFE_CLEANING))
print("DRY_RUN_CAMERA = " + str(DRY_RUN_CAMERA))
...
def zero_and_place_coupon():
    ...
    if DRY_RUN_COMPRESSION_TESTS:
        # simplified path: grab a clean coupon and place it directly on the
        # opentrons stand, skipping the 3 zero compression tests entirely
        armLock.acquire()
...
def run_test(param = None):
    ...
    if DRY_RUN_CHILLER:
        chiller_process = threading.Thread(target = lambda: None)
    else:
        chiller_process = threading.Thread(target=chiller.go_to_temperature, args=(bath_temp, ))
    chiller_process.start()
    # Background task 2: take a clean coupon, run the three zero tests on it (unless dry-running), then place it into the opentrons
    ...
    if DRY_RUN_KNIFE_CLEANING:
        knife_cleaning_process = threading.Thread(target = lambda: None)
    ...
    if not DRY_RUN_CAMERA:
        url.take_snapshot()

    if DRY_RUN_COMPRESSION_TESTS:
        # skip the membrane compression tests entirely - go straight from
        # the pre-test photo to the post-bath snapshot and discard
        xArm.open_camera_box()
        xArm.pick_up("coupon camera middle", pitch = False)
...
if __name__ == '__main__':
    # bound to a different port (8001) than the production server (8000) so
    # the two can't be confused with each other / accidentally collide
    server = HTTPServer(("169.254.46.48", 8001), RequestHandler)
    print("DRY RUN server listening on 169.254.46.48:8001")
